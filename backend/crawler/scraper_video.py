        ...
        if page is not None:
            ...
            try:
                ...
                resp = await page.goto(
                    view_url, timeout=PAGE_TIMEOUT, wait_until="domcontentloaded"
                )
                if resp and resp.ok:
                    text = await page.evaluate("() => document.body.innerText")
                    data = json.loads(text)
            ...
            try:
                ...
                if not mixin_key:
                    ...
                    if session_failures >= 3:
                        logger.warning(
                            "mixin_key 连续失败 %d 次 bv=%s, 放弃评论采集",
                            session_failures,
                            bv_id,
                        )
                        return comments, 0
                    logger.warning(
                        "mixin_key 获取失败 bv=%s, 强制换代理重试 (%d/3)",
                        bv_id,
                        session_failures,
                    )
                ...
                for _pn in range(pn, session_end):
                    ...
                    try:
                        ...
                        if code in (-101, 3, -6):
                            logger.warning(
                                "Cookie 已过期! code=%d bv=%s, 自动删除", code, bv_id
                            )
                        ...
                        # 检测到底: cursor.is_end 或返回数 < 15 (B站 reply API 每页最多30条)
                        cursor = data.get("data", {}).get("cursor", {})
                        if cursor.get("is_end") or len(replies) < 15:
                            pn = api_pages + 1
                            break
